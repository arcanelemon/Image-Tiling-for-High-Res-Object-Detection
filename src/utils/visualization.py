...
class BBoxVisualization():
    ...
    def draw_bboxes(self, img, boxes, confs, clss):
        ...
        for bb, cf, cl in zip(boxes, confs, clss):
            ...
            color = (0,0,255) if type(cl) == str else self.colors[int(cl)]
            cv2.rectangle(img, (x_min, y_min), (x_max, y_max), color, 2)
            txt_loc = (max(x_min+2, 0), max(y_min+2, 0))
            cf_txt = cf if type(cf) == str else '{:.2f}'.format(cf) 
            cls_txt = cl if type(cl) == str else self.cls_dict.get(int(cl), 'CLS{}'.format(cl))
            img = draw_boxed_text(img, cls_txt + ' ' + cf_txt, txt_loc, color)
        return img
